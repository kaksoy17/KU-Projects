...
bool isThereAnyCar(){ //If none of the lanes have cars, it returns false, true otherwise
	bool isThere = false;
	for(int i=0;i<4;i++){
		if(larray[i]->count != 0){
			isThere = true;
...
void *Police(){
	...
	while(simulationTimeCounter < simulationTime+1){
		...
		if(passed_car != NULL){
			...
			//printf("Car %d is passed from lane %d. Left cars: %d \n",passsed_carId, currentId, larray[currentId]->count); //Prints there is -1 as carId no car
...
int main(int argc, char *argv[]){
	...
	mainPointer = fopen("part3Log.txt", "w");
