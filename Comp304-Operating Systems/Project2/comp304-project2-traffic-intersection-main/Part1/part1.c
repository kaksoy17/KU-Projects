...
int chooseNextLane(int currId){ //This function takes the currId as  the ID of the current and returns the next lane which uses the intersection
	if(currId == -1){ // This will be work just initially
		return 0;
	}
	int currCars = larray[currId]->count;
	int array[4]={larray[0]->count,larray[2]->count,larray[1]->count,larray[3]->count}; //North,East,South,West instead of N,S,E,W to provide N > E > S > W 
	int maxLane = findMax(array,4);
	
	if((currCars != 0) && (array[maxLane] < 5)){ //If there are not 5 or more cars in other lanes and the current has cars
		return currId;
	}
...
int main(int argc, char *argv[]){
	...
	mainPointer = fopen("part1Log.txt", "w");
