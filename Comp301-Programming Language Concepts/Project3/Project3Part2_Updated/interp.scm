  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        ; #####################################################
        (letrec-nested-exp (p-name b-var b-count p-body main-body)
          (value-of main-body
        ...
        (call-nested-exp (rator rand count)
          (let ((proc (expval->proc (value-of rator env)))
            ...
            (apply-procedure proc (cons n arg)))
    ...
    (lambda (proc1 arg)
      (cases proc proc1
        (procedure (var body saved-env)
          (value-of body (extend-env var (cdr arg) saved-env)))
        ...
        ; #####################################################
        ;
        (nested-procedure (bvar count name body saved-env)
                          (let ((newC (num-val (+ (car arg) 1)))
                                (realArg (cdr arg)))
                            (begin
                              (recursive-displayer name (expval->num newC))
                              (value-of body (extend-env bvar realArg (extend-env count newC saved-env)))) ;;Extending the environment with n+1 for the next call if there will be
                          ))
